...
17. **productSlug**: This field represents a slug or identifier for the product, which is utilized in the URLs of the store. The product slug serves as a reference in the URL, contributing to search engine optimization (SEO) efforts.
...
18. **productImages_id_1 (Required)**: Represents the unique identifier for the image. This id is necessary to save the image inside VTEX.

19. **productImages_url_1 (Required)**: Represents the URL of the image `productImages_id_1`. You have the flexibility to use external URLs, such as those from Google Drive.
...
20. **productImages_alt_1**: Describes the alternative text for the image `productImages_id_1`.

21. **productImages_id_2 … productImages_id_10**: Represents the unique identifier for the image. This id is necessary to save the image inside VTEX.

22. **productImages_url_2 … productImages_url_10**: Represents the URL of the image `productImages_id_X`. You have the flexibility to use external URLs, such as those from Google Drive.

23. **productImages_alt_2 … productImages_alt_10**: Describes the alternative text for the image `productImages_id_X`.

24. **skuId**: The skuId serves as the distinctive identifier for the SKU. This unique identifier is assigned to each SKU within the system. Importantly, **when creating new skus, it is crucial to omit this value**. The system will autonomously generate a unique `skuId` for new products during the creation process.
...
25. **skuName**: The `skuName` represents the distinct name assigned to the SKU. This serves as a clear and concise label for the SKU, contributing to effective product differentiation. It is imperative to use straightforward language, avoiding complex terms or languages. The `skuName` is a crucial element for SEO optimization and must adhere to the 150-character limit.

26. **skuExternalId**: The `skuExternalId` serves as an external identifier for the SKU. This unique reference code is crafted to enhance the organization of the store. It provides a valuable means of identification for the SKU in external systems or processes.

27. **skuEan**: The `skuEan` is the EAN (International Article Number) assigned to the SKU. This unique barcode comprises up to 13 numeric characters, providing a standardized identification code for the SKU. The EAN is crucial for accurate and efficient tracking of the SKU across various systems.

28. **skuManufacturerCode**: The `skuManufacturerCode` is the manufacturer code specific to the SKU. This code serves as a reference within the store, facilitating streamlined organization and management of SKU-related information. It is essential for associating the SKU with its respective manufacturer.

29. **skuIsActive**: The `skuIsActive` field indicates the activity status of the SKU. It is a boolean attribute, meaning it accepts either `true` or `false` values.
...
30. **skuWeight**: The `skuWeight` field represents the weight of the SKU and is specified in grams. It is a numerical attribute, allowing for weights lighter than 1000 grams.

31. **skuWidth (Required)**: The `skuWidth` field represents the width of the SKU.

    - **Example**: `1.5`

32. **skuHeight (Required)**: The `skuHeight` field represents the height of the SKU.

    - **Example**: `2.0`

33. **skuLength (Required)**: The `skuLength` field represents the length of the SKU.
...
34. **skuSpecs_name_1 (Required)**: Represents the name of the SKU's specification, providing a distinct attribute for the SKU.

    - **Example**: `Color`

35. **skuSpecs_value_1 (Required)**: Specifies the value associated with the particular specification defined on `skuSpecs_name_1`, contributing to defining the unique characteristics of the SKU.
...
36. **skuSpecs_name_2**: Represents the name of the SKU's specification, providing a distinct attribute for the SKU.

    - **Example**: `Size`

37. **skuSpecs_value_2**: Specifies the value associated with the particular specification defined on `skuSpecs_name_2`, contributing to defining the unique characteristics of the SKU.
...
39. **productTransportModal**: Represents the transport modal used for transporting the product.

40. **productTaxCode**: Signifies the tax code associated with the product.

41. **productDescription**: The `productDescription` field is used to provide a detailed description of the product. It supports HTML formatting, allowing you to structure the content as needed.
